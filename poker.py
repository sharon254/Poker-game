...
king = 5
ace = 6
names = {nine: "9", ten: "10", jack: "10", queen: "Q", king: "K", ace: "A"}
player_score = 0
computer_score = 0
...
def throws():
    ...
    while True:
        rerolls = input("How many dice do you want to true again ? ")
        try:
            if rerolls in (1, 2, 3, 4, 5):
                break
        except ValueError:
            pass
        print: "Ooops! I didnt understand that!Please enter  1, 2, 3, 4, 5."
    if rerolls == 0:
        print("You will finish with", result)
    else:
        roll_number = rerolls
        dice_rerolls = roll(roll_number)
        dice_changes = range(rerolls)
        print("Enter the number of the dice to rerolll: ")
        iterations = 0
